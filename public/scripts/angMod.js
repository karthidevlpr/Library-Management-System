...
function MenuCtrl($scope,$state,route,userDetail){
    ...
    $scope.logout = function(){
        ...
        route.save({entity: "logout"}, function (response) {
...
function UserListCtrl($scope,route,$state){
    ...
    $scope.ctrlObj = {role:'ADMIN'};
    
    var GetUsers = function (role) {
        route.list({entity: "users",id:role}, function (response) {
    ...
    $scope.editUser = function(item){
        $state.go("menu.editUser",{id:item._id})
    };
    $scope.changeStatus = function(item,status){
        route.update({entity: "user",id:item._id,action:'statusChange',status:status}, function (response) {
            GetUsers($scope.ctrlObj.role)
        }, function (error) {
            ErrorHandling1(error,$state)
        });
...
function UserCtrl($scope,route,$state,$stateParams){
  ...
  $scope.user = {role:'ADMIN'};
    ...
    $scope.userSave = function(){
        route.save({entity: "user"},$scope.user, function (response) {
            console.log(response);
            $state.go('menu.users')
        }, function (error) {
            console.log(error)
...
function UserEditCtrl($stateParams,$scope,route,$state){
    ...
    var GetUsers = function () {
        route.get({entity: "user",id:$stateParams.id}, function (response) {
            $scope.user = response;
        }, function (error) {
            ErrorHandling1(error,$state)
        });
    ...
    $scope.userUpdate = function (user) {
        ...
        route.update({entity: "user",id:user._id},$scope.user, function (response) {
            console.log(response);
            $state.go('menu.users')
        }, function (error) {
            switch (error.status) {
...
function BookListCtrl($scope,route,$state){
    ...
    var GetBooks = function () {
        route.list({entity: "books"}, function (response) {
    ...
    $scope.editBook = function(item){
        $state.go("menu.editBook",{id:item._id})
    };
    $scope.changeStatus = function(item,status){
        route.update({entity: "book",id:item._id,action:'statusChange',status:status}, function (response) {
            GetBooks()
        }, function (error) {
            ErrorHandling1(error,$state)
        });
...
function BookCtrl($scope,route,$state,$stateParams){
    ...
    $scope.bookSave = function(){
        route.save({entity: "book"},$scope.book, function (response) {
            console.log(response);
            $state.go('menu.books')
        }, function (error) {
            console.log(error)
...
function BookEditCtrl($stateParams,$scope,route,$state){
    ...
    var GetBook = function () {
        route.get({entity: "book",id:$stateParams.id}, function (response) {
            $scope.book = response;
        }, function (error) {
            ErrorHandling1(error,$state)
        });
    ...
    $scope.bookUpdate = function (book) {
        ...
        route.update({entity: "book",id:book._id},$scope.book, function (response) {
            console.log(response);
            $state.go('menu.books')
        }, function (error) {
            switch (error.status) {
...
function TransactionListCtrl($scope,route,$state){
    ...
    var GetTransactions = function () {
        route.list({entity: "transactions"}, function (response) {
    ...
    $scope.changeStatus = function(item){
        route.update({entity: "transaction",id:item._id,action:'changeStatus'}, function (response) {
            console.log(response)
            GetTransactions()
        }, function (error) {
            ErrorHandling1(error,$state)
...
function TransactionCtrl($scope,route,$state,$stateParams){
    ...
    $scope.transactionSave = function(transaction){
        ...
        route.save({entity: "transaction"},transaction, function (response) {
            console.log(response);
